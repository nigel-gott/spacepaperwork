...
class GooseUser(AbstractUser):
    ...
    def egg_transactions(self):
        return EggTransaction.objects.filter(
            counterparty_discord_username=self.discord_username()
        )
...
class Fleet(models.Model):
    ...
    loot_type = models.ForeignKey(LootType, on_delete=models.CASCADE, null=True, blank=True)
    ...
    def isk_and_eggs_balance(self):
        isk = to_isk(
            model_sum(
                IskTransaction.objects.filter(item__loot_group__bucket__fleet=self.id),
                "isk",
            )
        )
        eggs = to_isk(
            model_sum(
                EggTransaction.objects.filter(item__loot_group__bucket__fleet=self.id),
                "eggs",
            )
        )
        return isk + eggs
    ...
    def estimated_profit(self):
        total = to_isk(
            InventoryItem.objects.filter(
                loot_group__fleet_anom__fleet=self.id
            ).aggregate(
                estimated_profit_sum=Sum(
                    Coalesce(F("item__cached_lowest_sell"), 0) * F("quantity")
                    + Coalesce(F("marketorder__listed_at_price"), 0)
...
class ItemFilterGroup(models.Model):
    ...
    def clean(self):
        if self.min_level and self.max_level and self.min_level > self.max_level:
            raise ValidationError(
                _("Min level must be less than or equal to max level")
            )
...
class AnomType(models.Model):
    ...
    def clean(self):
        if self.level < 6 and self.type == "Deadspace":
            raise ValidationError(_("Deadspaces cannot be lower than level 6"))
        if self.level < 6 and self.type.startswith("Condensed"):
            raise ValidationError(_("Condenesed Belts cannot be lower than level 6"))
        if self.type.startswith("Condensed") and self.faction != "Asteroids":
            raise ValidationError(_("A Belt must be in the Asteroids Faction"))
        if self.type.startswith("PvP") and self.faction != "PvP":
            raise ValidationError(_("A PvP type must be in the PvP Faction"))

    def scored_item_filter_groups(self, name_filter=None):
        groups = ItemFilterGroup.objects.filter(
            Q(faction__isnull=True)
            | Q(faction=self.faction) & Q(anom_type__isnull=True)
            | Q(anom_type=self.type) & Q(min_level__isnull=True)
            | Q(min_level__lte=self.level) & Q(max_level__isnull=True)
            | Q(max_level__gte=self.level)
        )
        if name_filter:
...
class ItemFilter(models.Model):
    ...
    item_sub_type = models.ForeignKey(
        ItemSubType, on_delete=models.CASCADE, null=True, blank=True
    )
...
class LootBucket(models.Model):
    ...
    def isk_and_eggs_balance(self):
        isk = to_isk(
            model_sum(
                IskTransaction.objects.filter(item__loot_group__bucket=self.id), "isk"
            )
        )
        eggs = to_isk(
            model_sum(
                EggTransaction.objects.filter(item__loot_group__bucket=self.id), "eggs"
            )
        )
        return isk + eggs
...
class LootGroup(models.Model):
    ...
    def isk_and_eggs_balance(self):
        isk = to_isk(
            model_sum(IskTransaction.objects.filter(item__loot_group=self.id), "isk")
        )
        eggs = to_isk(
            model_sum(EggTransaction.objects.filter(item__loot_group=self.id), "eggs")
        )
        return isk + eggs

    def estimated_profit(self):
        return to_isk(
            InventoryItem.objects.filter(loot_group=self.id).aggregate(
                estimated_profit_sum=Sum(
...
class StackedInventoryItem(models.Model):
    ...
    def marketorders(self):
        return MarketOrder.objects.filter(item__stack=self.id).order_by(
            "item__created_at"
        )
...
class InventoryItem(models.Model):
    ...
    def add_isk_transaction(self, isk, transaction_type, quantity, notes, user):
        if not self.has_admin(user):
            return (
                False,
                f"{user} doesn't have permissions to add an isk transaction for {self}",
            )
        else:
...
class MarketOrder(models.Model):
    ...
    transaction_tax = DecimalField(max_digits=5, decimal_places=2)
    broker_fee = DecimalField(max_digits=5, decimal_places=2)
    ...
    def change_price(self, new_price, broker_fee, changing_user):
        ...
        with transaction.atomic():
            ...
            if new_price == old_price:
                ...
                self.full_clean()
                self.save()
                return self.item.add_isk_transaction(
                    -fee, "price_change_broker_fee", self.quantity, notes, changing_user
                )
