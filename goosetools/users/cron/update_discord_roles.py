        ...
        for uid, user in uid_to_user.items():
            if user.status == "approved":
                ...
                if user.groups():
                    ...
                    output = (
                        output
                        + f"{user.display_name()}({characters}) and has no groups\n"
                    )
        output = output + (f"Processed {users_processed} users.\n")
        return output
    else:
        return "Your Discord Connection is Broken, please go to Admin->Discord Settings to fix."
