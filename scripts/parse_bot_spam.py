...
class Character:
    ...
    def __eq__(self, obj):
        return isinstance(obj, Character) and obj.pk == self.pk and obj.ingame_name == self.ingame_name and obj.corp == self.corp and self.discord_user_pk == obj.discord_user_pk
...
class DiscordUser:
    ...
    def __str__(self):
        return f"pk({self.pk}), uid({self.uid}), hash({self.avatar_hash}), username({self.username}), chars({self.characters})"

    def __eq__(self, obj):
        return isinstance(obj, DiscordUser) and obj.pk == self.pk and obj.uid == self.uid and obj.avatar_hash == self.avatar_hash and self.username == obj.username
        ...
        for model in existing_models:
            if model['model'] == 'core.discorduser':
                ...
                if uid in existing_characters:
                    ...
                    existing_users[uid] = DiscordUser(model['pk'], fields['uid'], fields['avatar_hash'], fields['username'])
        ...
        for model in existing_models:
            if model['model'] == 'core.character':
                ...
                if ingame_name in existing_characters:
                    ...
                    char = Character(model['pk'], fields['ingame_name'], fields['corp'], fields['discord_user']) 
    ...
    with open(os.path.join(os.curdir, "../misc_data/auth.csv"), "r", newline="") as auth_csv:
        ...
        for line in lines:
            ...
            if ingame_name not in chars:
                chars[ingame_name] = Character(char_pk, ingame_name, corp, users[uid].pk)
...
def load_chat_data(chars, users, char_pk, user_pk):
    discord_id_to_characters, author_info = parse_chat_data()
    for uid, chat_data in discord_id_to_characters.items():
        if uid not in users:
            new_user = DiscordUser(user_pk, uid, url_to_hash(author_info[uid]['avatar_url']), author_info[uid]['discord_name']) 
            users[uid] = new_user
            user_pk = user_pk + 1
        elif users[uid].avatar_hash is None:
            new_user = DiscordUser(users[uid].pk, uid, url_to_hash(author_info[uid]['avatar_url']), author_info[uid]['discord_name']) 
        ...
        for ingame_name, corp in chat_data.items():
            if ingame_name not in chars:
                ...
                new_character = Character(chars[ingame_name].pk, ingame_name, corp, users[uid].pk)
